"""
Mass fits to data to get the yields, using the alternate bkg model
...
def _fit(
    ...
    fig, axes = plotting.alt_bkg_simul(
        rs_count,
        rs_err,
        ws_count,
        ws_err,
...
def main(
    *,
    ...
    plot_dir = mass_util.alt_plot_dir(bdt_cut, efficiency, phsp_bin)

    # For writing the result
    yield_file_path = mass_util.alt_yield_file(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create mass fit plots using the alternate background model"
    )
    parser.add_argument(
        "year",
        type=str,
