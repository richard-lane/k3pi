...
def bes_chi2(
    ...
    if not _bes_cov_mat_exists():
        print(
            f"Get the BES-III covariance matrix ROOT file from somewhere, and save it to {_bes_cov_mat_path()}",
            file=sys.stderr,
        )
        raise FileNotFoundError(_bes_cov_mat_path())
    if not _bes_lib_exists():
        print(f"Create the BES library by running the build script", file=sys.stderr)
